...
public class DungeonGame {
    ...
    public int calculateMinimumHP(int[][] d) {
        ...
        if (d[0][0] < 1) {
            minHeath = Math.max(Math.abs(d[0][0])+1, minHeath);
        }
        queue.add(new Cell(0, 0, d[0][0], minHeath));
        pathMap.put("00", true);
        while (!queue.isEmpty()) {
            Cell cell = queue.poll();
            if (cell.x == d.length-1 && cell.y == d[0].length-1) {
                res = Math.min(cell.minHealth, res);
            }
            else {
                int[] dx = {0, 1};
                int[] dy = {1, 0};
                for (int k = 0; k<2; k++) {
                    int newX = cell.x+dx[k];
                    int newY = cell.y+dy[k];
                    if (isValid(newX, newY, d)) {
                        int newCost = cell.costFromStart + d[newX][newY];
                        int health = cell.minHealth;
                        if (health + newCost <= 0) {
                            health = Math.max(Math.abs(newCost)+1, health);
                        }
                        Cell next = new Cell(newX, newY, newCost, health);
                        queue.add(next);
                    }
                }
            }
    ...
    private boolean isValid(int x, int y, int[][] d) {
        return x >= 0 && y >= 0 && x < d.length && y < d[0].length;
    }
    ...
    class Cell {
        ...
        public Cell(int x, int y, int costFromStart, int minHealth) {
            this.x = x;
            this.y = y;
    ...
    public int calculateMinimumHP2(int[][] d) {
        if (d == null || d.length == 0 || d[0].length == 0) {
            return 1;
        }
        Integer[][] sol = new Integer[d.length][d[0].length];
        return calc(0, 0, d, sol);

    }

    private int calc(int i, int j, int[][] d, Integer[][] sol) {
        if (i == d.length-1 && j == d[0].length-1) {
            int res =  d[i][j] < 0 ? Math.abs(d[i][j])+1 : 1;
            sol[i][j] = res;
            return res;
        }
        if (sol[i][j] != null) {
        ...
        if (i == d.length-1) {
            return sol[i][j] = Math.max(calc(i, j + 1, d, sol) - d[i][j], 1);
        }
        if (j == d[0].length-1) {
            return sol[i][j] = Math.max(calc(i+1, j, d, sol) - d[i][j], 1);
        }
        int fr = Math.max(calc(i, j + 1, d, sol) - d[i][j], 1);
        int fd = Math.max(calc(i+1, j, d, sol) - d[i][j], 1);
        return sol[i][j] = Math.min(fr, fd);
    }

    @Test
    public void test6() {
        int[][] a = {{-2, -3, 3},{-5,-10,1},{10,30,-5}};
        assertEquals(calculateMinimumHP2(a), 7);
        int[] ab = {1,2};
    ...
    public void test2() {
        int[][] a = {{}};
        assertEquals(calculateMinimumHP(a), 1);
    }
